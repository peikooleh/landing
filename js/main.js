(function () {
  ...
  const translations = {
    ru: {
      ...
      'hero.subtitle':
        '<span class="brand-word">MOYAMOVA</span> — простой и честный тренажёр иностранных слов. Базовый функционал остаётся бесплатным, а PRO-опции можно докупить один раз за 9.99&nbsp;€ — без подписок и рекламы.',
      'hero.currentLang':
      ...
      'screens.lead':
        '<span class="brand-word">MOYAMOVA</span> — чистый тренажёр без новостных лент, рейтингов и лишних разделов. Только вы, карточки со словами, удобные списки словарей, избранного и ошибок — и никакой рекламы или подписок.',
      'screens.themes.text':
        'Адаптивный интерфейс под светлую и тёмную темы. Тренажёр удобно использовать на телефоне, планшете и ноутбуке — днём и вечером.',
      'screens.light.label': 'Светлая тема',
      'screens.dark.label': 'Тёмная тема',
      'screens.card2.title': 'Статистика и прогресс внутри тренажёра',
      'screens.card2.text':
        'Словари разбиты по категориям и частям речи, важные слова можно добавлять в избранное, а сложные автоматически попадают в список ошибок для отдельного повтора.',
      'screens.card3.title': 'Отдельные наборы ошибок и избранного',
      'screens.card3.text':
        'Озвучка слов и примеров, живые примеры в предложениях, синонимы и антонимы, а также экспорт и импорт словарей помогают учиться в своём ритме и не терять прогресс.',
      'screens.card4.text':
      ...
      'faq.q1.text':
        'Да, тренажёр можно использовать бесплатно для некоммерческого личного обучения. Основные словари, карточки, примеры, озвучка и офлайн-прогресс доступны без оплаты. В тренажёре есть платные PRO-опции, которые расширяют функциональность и удобство обучения. PRO покупается один раз за 9.99&nbsp;€ и остаётся с вами навсегда — без подписок и скрытых списаний.',
      ...
      'faq.q6.text':
        'PRO-версия открывает дополнительные опции и инструменты для более удобного и продвинутого обучения. Модель простая: вы один раз оплачиваете PRO за 9.99&nbsp;€, после чего расширенные функции остаются с вами навсегда. Никаких подписок, ежемесячных платежей и рекламы.',
      ...
      'screens.features.fav.label': 'Избранное',
      'screens.features.errors.label': 'Мои ошибки',
      'screens.perks.audio.label': 'Озвучка',
      'screens.perks.examples.label': 'Примеры',
      'screens.perks.synonyms.label': 'Синонимы',
      'screens.perks.antonyms.label': 'Антонимы',
      'screens.perks.export.label': 'Экспорт и импорт',

      'pro.title': 'PRO-режим: разовая покупка навсегда',
      'pro.lead':
        'Базовый тренажёр остаётся бесплатным. PRO-опции покупаются один раз за 9.99&nbsp;€ и остаются с вами навсегда — без подписок, ежемесячных платежей и рекламы.',
      'pro.point1.title': 'Честная модель оплаты',
      'pro.point1.text':
        'Вы один раз оплачиваете PRO за 9.99&nbsp;€. Никаких автосписаний, подписок и “сюрпризов” в конце месяца.',
      'pro.point2.title': 'Расширенные возможности',
      'pro.point2.text':
        'PRO открывает дополнительные опции для более удобного и продвинутого обучения. Базовый тренажёр при этом остаётся бесплатным.',
      'pro.point3.title': 'Простая активация',
      'pro.point3.text':
        'Покупка и активация происходят прямо внутри тренажёра. Оплата через PayPal, после чего PRO включается автоматически.',
      'pro.step1.label': 'Где нажать',
      'pro.step2.label': 'Оплата',
      'pro.step3.label': 'Активация',
      'pro.cta': 'Узнать о PRO внутри тренажёра',
      'pro.note': '* PRO покупается один раз за 9.99&nbsp;€ — без подписок и рекламы',

      'updates.title': 'Что нового в MOYAMOVA',
      'updates.lead':
        'Коротко рассказываем про новые словари, языки, функции и улучшения тренажёра.',
      'updates.slide1.label': 'Новый словарь',
      'updates.slide2.label': 'Новая функция',
      'updates.slide3.label': 'Улучшения',
      'updates.text':
        'При каждом крупном обновлении мы добавляем сюда короткую заметку и скрин. Так можно быстро понять, что поменялось в тренажёре.',
},
    uk: {
